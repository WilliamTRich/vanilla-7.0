...
public static class SLogConfig
{
    ...
    public const LogLevel MinLevel = LogLevel.Debug; //Default = LogLevel.Debug | Minimum logging level.
    public const bool ExportErrors = false; //Default = false | Should we export error logs to a file?
    public const int SleepTime = 500; //Default = 500 | Time in MS between sending all queued messages.
}
            ...
            while (_messages.TryDequeue(out var work))
            {
                ...
                if (SLogConfig.ExportErrors && work.Level == LogLevel.Error)
                {
                    await File.AppendAllTextAsync(
                        Path.Combine(_path, $"error-log-{DateTime.Now.ToString("yyyy-MM-dd---HH-mm-ss-fffffff")}.txt"), "\n" + work.Message);
                }

                if (work.Level == LogLevel.Fatal)
                {
                    await File.AppendAllTextAsync(
                        Path.Combine(_path, $"fatal-log-{DateTime.Now.ToString("yyyy-MM-dd---HH-mm-ss-fffffff")}.txt"), "\n" + work.Message);
                }
            }
