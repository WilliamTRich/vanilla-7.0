...
public static class Database
{
    ...
    public static int GetStars(AccountModel acc)
    {
        ...
        foreach (var classStat in acc.Stats.ClassStats)
            for (var i = 0; i < Constants.GetStarsLength(); i++) {
                if (classStat.BestFame >= Constants.GetStar(i))
                    starCount++;
            }
        return starCount;
    }
    ...
    public static bool IsAccountInUse(AccountModel acc, bool clientExistsOnGameServer = false)
    {
        var accountInUse = acc.Connected && clientExistsOnGameServer;//Manager.GetClient(acc.Id) != null;
...
;            }
    ...
    public static CharacterModel CreateCharacter(AccountModel acc, int classType, int skinType)
    {
        ...
        var character = new CharacterModel(acc.Id, newId)
        {
            ...
            SkinType = skinType,
            HasBackpack = false,
            HealthPotions = Constants.MaxPotions,
            MagicPotions = Constants.MaxPotions,
